...
<?php 
    ...
    */
    class Database
    {
        ...
        {
            $this->link = mysqli_connect("localhost","root","","orderfoodlrv") or die ();
        ...
        public function countTable($ma,$table)
        {
         ...
         */
        public function delete ($table ,$ma, $id )
        {
            $sql = "DELETE FROM {$table} WHERE {$ma} = {$id} "."LIMIT 1";
        ...
        public function deletewhere($table,$data = array())
        {
            ...
            {
                $id = intval($id);
                $sql = "DELETE FROM {$table} WHERE mamonan = $id ";
                mysqli_query($this->link,$sql) or die (" Lỗi Truy Vấn delete   --- " .mysqli_error($this->link));
            }
            return true;
        }
        ...
        public function fetchID($table ,$ma, $id )
        {
        ...
        public function fetchtat($mamonan)
        {
            $sql = "SELECT * FROM monan,quan_an,loaimonan WHERE monan.ma_qa = quan_an.ma_qa AND monan.maloai = loaimonan.maloai AND mamonan = {$mamonan}" ;
            $result = mysqli_query($this->link,$sql) or die("Lỗi Truy Vấn fetchAll " .mysqli_error($this->link));
            $data = [];
            if( $result)
            {
        ...
        public  function fetchJones($table,$sql,$total,$page,$row ,$pagi = false )
        {
